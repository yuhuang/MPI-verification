...
Based on \lemmaref{lemma:rst}, we further use $\mathcal{E}$$(\mathcal{RS}(\mathit{p}))$ to represent the equivalent classes for all the legal histories in $\mathcal{RS}_\mathit{p}$. If a equivalent class only consists of zero buffer legal histories, we call it zero buffer equivalent class. The following theorem states that a representative exists for each zero buffer equivalent class. 
