...
Based on \defref{def:projection_process} and \defref{def:projection_receive}, a legal history can be further projected to both receive responses $R$ and process $P$ that produces a sequences of $R$ on process $P$. We use $\mathcal{H} | _{R,P}$ to represent this projection. The equivalency relation relies on $\mathcal{H} | _{R,P}$.
...
The following lemma is essential to proving \defref{def:er} is a valid equivalency relation.
