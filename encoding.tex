...
$\mathit{order}_\mathtt{e1} \mathrm{\prec_\mathtt{HB}} \mathit{order}_\mathtt{e2}$, is a partial order over two event orders $\mathit{order}_\mathtt{e1}$ and $\mathit{order}_\mathtt{e2}$ such that a logic formula $\mathit{order}_\mathtt{e1} <  \mathit{order}_\mathtt{e2}$ is constrained. 
\label{def:hb}
\end{definition}
