...
void write_plain(int time) {
  ...
  sprintf(filename, "./badout/out_%d", time);
...
void update(int time) {
  ...
  for (i=1; i<nx-1; i++)
	...
	+ k*(u[old][i+1][j] + u[old][i-1][j]
	     + u[old][i][j+1] + u[old][i][j-1] - 3*u[old][i][j]);
}
