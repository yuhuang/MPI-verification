...
A problem in any MPI program is zero buffer incompatibility. An execution on a zero buffer incompatible program causes deadlock. Note that the zero buffer incompatibility is not equivalent to deadlock that may be caused by reasons other than buffering. The zero buffer incompatibility is essential to any MPI application since it is difficult for a developer to predict. 
