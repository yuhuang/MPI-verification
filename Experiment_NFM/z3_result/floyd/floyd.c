...
fprintf(fp, "T%d_%d: assume(= tab_%d_%d_%d_%d %d)\n", rank,c++,i,j,rank,ssa_tab[i][j]++, tab[i][j]);
		//printf("%d: tab[%d][%d] = %d\n", rank, i,j, tab[i][j]);
...
int ite;
    ...
    for (k = 0; k < n; k++) {
        if ( rank != owner(k) ){
            tab[k] = spare;
	for(ite=0; ite< n;ite++){
		fprintf(fp, "T%d_%d: assume(= tab_%d_%d_%d_%d %d)\n", rank,c++,k,ite,rank,ssa_tab[k][ite]++, 9999);
		}
        }
        ...
        if (rank == owner(k)) { // sending
			...
			fprintf(fp, "T%d_%d: snd(%d, tab_%d_%d_%d_%d)\n", rank,c++,i,k,0,rank,ssa_tab[k][0]-1);
			fprintf(fp, "T%d_%d: wait(T%d_%d)\n", rank, c, rank, c-1);
			c++;
		}
	} else {	
		MPI_Recv(tab[k], n, MPI_INTEGER, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);	
		fprintf(fp, "T%d_%d: rcv(%d, tab_%d_%d_%d_%d)\n", rank,c++,rank,k,0,rank,ssa_tab[k][0]++);
		fprintf(fp, "T%d_%d: wait(T%d_%d)\n", rank, c, rank, c-1);
		c++;	
	}
		...
		for (j = 0; j < n; j++) {
		    tmp = tab[i][k] + tab[k][j];
		    fprintf(fp, "T%d_%d: assume(= tmp_%d_%d (+ tab_%d_%d_%d_%d tab_%d_%d_%d_%d))\n", rank,c++,rank,ssa_tmp++,i,k,rank,ssa_tab[i][k]-1, k,j,rank,ssa_tab[k][j]);
		    if (tmp < tab[i][j]) {
			fprintf(fp, "T%d_%d: assume(< tmp_%d_%d tab_%d_%d_%d_%d)\n",rank,c++,rank,ssa_tmp-1,i,j,rank,ssa_tab[i][j]-1);
			tab[i][j] = tmp;
			fprintf(fp, "T%d_%d: assume(= tab_%d_%d_%d_%d tmp_%d_%d)\n",rank,c++,i,j,rank,ssa_tab[i][j]++,rank,ssa_tmp-1);

fprintf(fp, "T%d_%d: assert(<= tab_%d_%d_%d_%d tab_%d_%d_%d_%d)\n",rank,c++,i,j,rank,ssa_tab[i][j]-1,i,j,rank,ssa_tab[i][j]-2);
		    }
		}
            }
        }
