...
double worker(double a, double f(double),int c, int rank, FILE* fp) {
 ...
 int ssa_result =0;
  ...
  //  while (1) {
    ...
    fprintf(fp, "T%d_%d: rcv(%d, task_%d_%d)\n", rank,c++,rank, rank, ssa_task++);
...
fprintf(fp, "T%d_%d: snd(%d, result_%d_%d)\n", rank,c++,0,rank,ssa_result);
fprintf(fp, "T%d_%d: wait(T%d_%d)\n", rank, c, rank, c-1);
c++;
    //} else {
    //  break;
...
double manager(double f(double),int c, FILE* fp) {
  int i, task = 0;
  MPI_Status status;
  double result, answer = 0.0;
	...
	int ssa_task = 0;
    	int ssa_result =0;
	int ssa_answer =0;
fprintf(fp, "T%d_%d: assume(= task_%d 0)\n", 0,c++, ssa_task++);
fprintf(fp, "T%d_%d: assume(= result_%d 0)\n", 0,c++, ssa_result++);
fprintf(fp, "T%d_%d: assume(= answer_%d 0)\n", 0,c++, ssa_answer++);
  for (i=1; i<nprocs; i++) {
    MPI_Send(&task, 1, MPI_INT, i, WORK_TAG, MPI_COMM_WORLD);
fprintf(fp, "T%d_%d: snd(%d, task_%d)\n", 0,c++,i,ssa_task-1);
fprintf(fp, "T%d_%d: wait(T%d_%d)\n", 0, c, 0, c-1);
c++;
    task++;
fprintf(fp, "T%d_%d: assume(= task_%d (+ task_%d 1))\n", 0,c++,ssa_task++,ssa_task-1);
...
fprintf(fp, "T%d_%d: rcv(%d, result_%d_%d)\n", 0,c++,0, 0, ssa_result++);
fprintf(fp, "T%d_%d: wait(T%d_%d)\n", 0, c, 0, c-1);
c++;
//printf("resut: %f\n", result);
    /* MPI_Send(NULL, 0, MPI_INT, status.MPI_SOURCE, TERMINATE_TAG, MPI_COMM_WORLD); */
    answer += result;
fprintf(fp, "T%d_%d: assume(= answer_%d (+ result_%d 1))\n", 0,c++,ssa_answer++,ssa_result-1);

  }
assert(answer==0);
fprintf(fp, "T%d_%d: assert(= answer_%d 0)\n", 0, c++, ssa_answer-1);
  //printf("answer = %f\n",answer);
  return answer;
}

/* called in collective fashion */
double integral(double a, double b, double tolerance, double f(double), int c, int rank, FILE* fp) {
  ...
  if (rank == 0) {
    return manager(f,c,fp);
  } else {
    return worker(a,f,c,rank,fp);
  }
}
...
FILE* fp = fopen(address,"w");	
  ...
  result = integral(0, pi, 1, f1,c,rank,fp);
